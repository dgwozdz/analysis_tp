...
`r skewness(log(data.videos[, "V168"]))` suggests that the distribution is skewed to
the right.
...
```{r eval = T, fig.align = "center", fig.height = 7.5, fig.width = 7.5}
for(i in 1:24){
...
The model which minimizes mRSE is the
`r which(mRSE.single.var$RMSE.train == min(mRSE.single.var$RMSE.train))`th one, so it 
is the one with log-transformed V24
